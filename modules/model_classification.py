...
class Flatten(nn.Module):
    '''
    Flattens multi-dimensional array (N,*,*,*,...) into a vector (N,*).
    '''
...
class SimpleNet(nn.Module):
    '''
    The simplest CNN implementation for image classification task.
    Contains of three blocks Conv+MaxPool+ReLU and two dense layers.
    '''
    def __init__(self):
    ...
    def get_latent(self,x):
        '''
        Returns inner representation of images for the 2nd approach of the image extraction task.
        '''        
        x = self.relu1(self.mp1(self.conv1(x)))
        x = self.relu2(self.mp2(self.conv2(x)))  
        x = self.relu3(self.mp3(self.conv3(x)))          
...
def training_process(net_, X_train, y_train, X_val, y_val, n_epochs=20):
    '''
    Auxilliary function for compact training process in the __function__. Calculates and visualizes loss decay and also train and validation accuracy through epochs.
    '''
